...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ isOpen, onClose, darkMode = false }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className={`${darkMode ? 'bg-dark-card border-gray-700' : 'bg-white'} rounded-xl shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col`}>
        {/* Header */}
        <div className={`flex items-center justify-between p-6 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Dashboard Administrateur</h1>
        ...
        {/* Navigation Tabs */}
        <div className={`flex border-b ${darkMode ? 'border-gray-700 bg-dark-lighter' : 'border-gray-200 bg-gray-50'}`}>
          {tabs.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                ...
                className={`flex items-center space-x-3 px-6 py-4 text-sm font-medium transition-colors ${
                  activeTab === tab.id
                    ? darkMode
                      ? 'text-purple-400 bg-dark-card border-b-2 border-purple-400'
        ...
        <div className="flex-1 overflow-hidden">
          ...
          {activeTab === 'sessions' && (
            <div className={`p-6 text-center ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              ...
              <h3 className={`text-lg font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Gestion des Sessions</h3>
          ...
          {activeTab === 'analytics' && (
            <div className={`p-6 text-center ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              <BarChart3 className={`w-16 h-16 mx-auto mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-300'}`} />
              <h3 className={`text-lg font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Analytics</h3>
              <p>Fonctionnalité en cours de développement</p>
            </div>
          )}
          {activeTab === 'settings' && (
            <div className={`p-6 text-center ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              ...
              <h3 className={`text-lg font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Configuration</h3>
              <p>Fonctionnalité en cours de développement</p>
            </div>
          )}
        </div>
